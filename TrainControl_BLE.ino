...
COROUTINE(ledInput) {
  COROUTINE_LOOP() {
    if (lightState[HEADLIGHT] != RemoteXY.toggle1) {
      ...
      if (RemoteXY.toggle1 == 1) sprintf(RemoteXY.string, "Head light: ON");
      if (RemoteXY.toggle1 == 0) sprintf(RemoteXY.string, "Head light: OFF");
    }
    if (lightState[TAILLIGHT] != RemoteXY.toggle) {
      lightState[TAILLIGHT] = RemoteXY.toggle;
      if (RemoteXY.toggle == 1) sprintf(RemoteXY.string, "Tail light: ON");
      if (RemoteXY.toggle == 0) sprintf(RemoteXY.string, "Tail light: OFF");
    }
...
COROUTINE(motorOutput) {
  COROUTINE_LOOP() {
    ...
    if (RemoteXY.dir != dir) {
      dir = RemoteXY.dir;
      RemoteXY.speed = 0;
      if (dir == 0) sprintf(RemoteXY.string, "Reverse");
      if (dir == 1) sprintf(RemoteXY.string, "Forward");
    }
